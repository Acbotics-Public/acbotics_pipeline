...
class Sensor_Payload_IMU(UDP_Generic_Protocol):
    ...
    def encode(self, df):
        header = self.encode_header(
            timestamp=self.get_raw_timestamp(df.timestamp),
            id1=self.id1,
            id2=self.id2,
...
default_sensor_id_mappings = {str_2_bytes("G2"): Sensor_Payload_GPS_2}

import copy


class Routed_Sensor_Protocol(UDP_Generic_Protocol):
    def __init__(self, mappings=default_sensor_mapping):
        self.mappings = copy.copy(mappings)

    def decode(self, data):
        header = self.decode_header(data)
        if (header.ID1, header.ID2) in self.mappings:
            d = data[self.header_length_b :]
            protocol = self.mappings[(header.ID1, header.ID2)]
            dec = protocol.decode(d)
