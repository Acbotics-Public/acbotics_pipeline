...
class Fixture_CPP_Demo(Fixture):
    ...
    def configure_sense_rollover(self, min):
        self.cpp_blocks["SENSE_LOG"].set_rollover_min(min)

    def configure_acoustic_rollover(self, min):
        self.cpp_blocks["ACO_LOG"].set_rollover_min(min)
    ...
    def build(self):
        ...
        self.add_cpp_block(
            "SENSE_IN",
            ac.UdpSocketIn(
                self.args.use_mcast, self.args.iface_ip, self.args.sen_port, self.args.mcast_group
            ),
        )
        ...
        self.configure_sense_rollover(self.args.sen_rollover)
        self.configure_acoustic_rollover(self.args.aco_rollover)
...
def run():
    ...
    parser.add_argument(
        ...
        type=float,
        default=60,
        help="Create new sense file on the start of very n minutes.")
    parser.add_argument( 
        "--aco_rollover", 
        type=float, 
        default=5, 
        help="Create new aco file on the start of very n minutes.")
